...
from functions.functions import getBar
...
class fishing:
    ...
    async def fishing(self, ctx, *args):
        ...
        try:
            ...
            try:
                ...
                em = discord.Embed(title="You have some aids letters in your name and I cant save files with those sorry :(", description="", colour=0x3D3D5D)
                await self.bot.say(embed=em)
                return
    ...
    @commands.command(pass_context=True)
    async def throwrod(self, ctx, *, tag=None):
        await self.bot.send_typing(ctx.message.channel)
        try:
        ...
        if(cfg['items']['bait'] != 0):
            ...
            await self.bot.add_reaction(bait, '✅')
            await self.bot.add_reaction(bait, '❌')
            res = await self.bot.wait_for_reaction(('✅', '❌'), user=ctx.message.author, message=bait)
        ...
        for fishem in fishlst:
            await self.bot.add_reaction(tmp,fishem)
        ...
        await self.bot.edit_message(tmp, embed=em)

        res = await self.bot.wait_for_reaction(str(fishlst[react]), user=ctx.message.author, timeout=1.5, message=tmp)
        if(res != None):
            ...
            if(cfg['user']['lvl_progress'] >= 100):
                ...
                await self.bot.say(embed=emlvlup)
    ...
    async def shop(self, ctx, *args):
        ...
        for emoji in reactions:
            await self.bot.add_reaction(bait, emoji)

        res = await self.bot.wait_for_reaction(('1⃣', '2⃣', '3⃣', '4⃣', '❌'), user=ctx.message.author, message=bait)
        await self.bot.delete_message(bait)
